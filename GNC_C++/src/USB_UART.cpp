...
static int usb_uart_fd[USB_UART_MAX_BUS];
static float usb_uart_timeout_s[USB_UART_MAX_BUS];
static int usb_uart_shutdown_flag[USB_UART_MAX_BUS];


int USB_UART::begin(uint8_t new_bus,uint32_t baudrate,float timeout_s, int canonical_en, int stop_bits, int parity_en){
    printf("Initialising USB_UART...\n");
	...
	//Run
	int tmpfd, tenths;
	char buf[USB_UART_STRING_BUF];
	struct termios config;
	speed_t speed; //baudrate

	// sanity checks
	if(bus<0 || bus>USB_UART_MAX_BUS){
		fprintf(stderr,"USB_UART: Error. Bus must be between 0 & %d\n", USB_UART_MAX_BUS);
		return -1;
	}
	if(timeout_s<0.1f){
		fprintf(stderr,"USB_UART: Error. Timeout must be >=0.1 seconds\n");
		return -1;
	}
	if(stop_bits!=1 && stop_bits!=2){
		fprintf(stderr,"USB_UART: Error. Stop bits must be 1 or 2\n");
	}
	...
	case (50):
		...
		break;
	default:
		fprintf(stderr,"USB_UART: Error. Invalid baudrate. Please use a standard baudrate\n");
		return -1;
	}
	...
	snprintf(buf,sizeof(buf),"/dev/ttyUSB%d",bus);
	tmpfd = open(buf, O_RDWR | O_NOCTTY | O_NDELAY);
	if(tmpfd==-1){
		perror("USB_UART: Error while opening file descriptor");
	...
	if(tcgetattr(tmpfd,&config)==-1){
		fprintf(stderr,"USB_UART: Error. Cannot Get USB_UART Attributes\n");
		close(tmpfd);
		return -1;
	}
	...
	config.c_cc[VMIN]=USB_UART_MAX_READ_LEN;
	config.c_cc[VTIME] = tenths+1;

	// set speed in config struct
	if(cfsetispeed(&config, speed)==-1){
		perror("USB_UART: Error calling cfsetispeed");
		close(tmpfd);
		return -1;
	}
	if(cfsetospeed(&config, speed)==-1){
		perror("USB_UART: Error calling cfsetospeed");
		close(tmpfd);
		return -1;
	}
	...
	if(tcflush(tmpfd,TCIOFLUSH)==-1){
		perror("USB_UART: Error calling tcflush");
		close(tmpfd);
	}
	if(tcsetattr(tmpfd, TCSANOW, &config) < 0) {
		fprintf(stderr,"Cannot Set USB_UART%d Attributes\n", bus);
		close(usb_uart_fd[bus]);
		return -1;
	}
	if(tcflush(tmpfd,TCIOFLUSH)==-1){
		perror("USB_UART: Error calling tcflush");
	...
	if(fcntl(tmpfd, F_SETFL, 0)==-1){
		perror("USB_UART: Error calling fcntl");
		close(tmpfd);
		return -1;
	}
	if(tcflush(tmpfd,TCIOFLUSH)==-1){
	...
	usb_uart_fd[bus]=tmpfd;
	usb_uart_timeout_s[bus]=timeout_s;
	usb_uart_shutdown_flag[bus]=0;
	init=1;
	printf("USB_UART Initialised!\n");
	return 0;
}

int USB_UART::close_bus(){
	// sanity checks
	if(bus<0 || bus>USB_UART_MAX_BUS){
		fprintf(stderr,"USB_UART: Error. Bus must be between 0 & %d\n", USB_UART_MAX_BUS);
		return -1;
	}
	usb_uart_shutdown_flag[bus]=1;
	// if not initialized already, return
	if(usb_uart_fd[bus]==0) return 0;
	// flush and close
	tcflush(usb_uart_fd[bus],TCIOFLUSH);
	close(usb_uart_fd[bus]);
	usb_uart_fd[bus]=0;
	return 0;
}
...
int USB_UART::flush(){
	...
	if(usb_uart_fd[bus]==0){
		fprintf(stderr,"USB_UART: %d must be initialized first\n", bus);
		return -1;
	}
	if(tcflush(usb_uart_fd[bus],TCIOFLUSH)==-1){
		perror("USB_UART: Error flushing.");
		return -1;
	}
	return 0;
}

int USB_UART::write_bytes(uint8_t data[], size_t bytes){
	...
	if(bytes<1){
		fprintf(stderr,"USB_UART: Error. Number of bytes to send must be >1\n");
		return -1;
	}
	if(usb_uart_fd[bus]==0){
		fprintf(stderr,"USB_UART: %d must be initialized first\n", bus);
	...
	ret=write(usb_uart_fd[bus], data, bytes);
	if(ret==-1) perror("USB_UART: Error writting.");
	return ret;
}

int USB_UART::read_bytes(uint8_t data[],size_t bytes){
	//Exit if not initialised.
	if(!init)return 0;
	...
	if(bytes<1){
		fprintf(stderr,"USB_UART: Error. Number of bytes to read must be >=1\n");
		return -1;
	}
	if(usb_uart_fd[bus]==0){
		fprintf(stderr,"USB_UART: %d must be initialized first\n", bus);
	...
	if(bytes<=UART_MAX_READ_LEN){
		...
		ret = read(usb_uart_fd[bus], buf, bytes);
		return ret;
	}
	*/
	...
	timeout.tv_sec = (int)usb_uart_timeout_s[bus];
	timeout.tv_usec = (int)(1000000*fmod(usb_uart_timeout_s[bus],1));

	// exit the read loop once enough bytes have been read
	// or the the shutdown signal flag is set
	while((bytes_left>0) && usb_uart_shutdown_flag[bus]==0){
		FD_ZERO(&set); /* clear the set */
		FD_SET(usb_uart_fd[bus], &set); /* add our file descriptor to the set */
		ret = select(usb_uart_fd[bus] + 1, &set, NULL, NULL, &timeout);
		if(ret == -1){
			...
			if(errno!=EINTR){
				perror("USB_UART: Error calling select in read bytes.");
				return -1;
			}
			return bytes_read;
		}
			...
			if(bytes_left>USB_UART_MAX_READ_LEN)	bytes_to_read = USB_UART_MAX_READ_LEN;
			else bytes_to_read = bytes_left;
			ret=read(usb_uart_fd[bus], data+bytes_read, bytes_to_read);
			if(ret<0){
				perror("USB_UART: Error in read_bytes");
				return -1;
			}
			else if(ret>0){
...
int USB_UART::read_bytes(size_t bytes){
	return read_bytes(buffer_in,bytes);
}

int USB_UART::bytes_available(){
	//Exit if not initialised.
	if(!init)return 0;
	...
	if(ioctl(usb_uart_fd[bus], FIONREAD, &out)==-1){
		perror("USB_UART: Error calling ioctl");
		return -1;
	}
	return out;
}